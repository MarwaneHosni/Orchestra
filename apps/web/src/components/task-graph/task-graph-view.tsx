...
function useCollapsedPhases(): [Set<string>, (phase: string) => void] {
  const [collapsed, setCollapsed] = useState<Set<string>>(() => {
    ...
    try {
      const s = localStorage.getItem("task-graph-collapsed");
  ...
  const toggle = useCallback((phase: string) => {
    setCollapsed((prev) => {
      ...
      try {
        localStorage.setItem("task-graph-collapsed", JSON.stringify([...next]));
      } catch {
...
export function TaskGraphView({ sessionId }: { sessionId: string }) {
  ...
  const handleResolve = useCallback(async () => {
    try {
      ...
      console.error("Failed to refresh task graph", e);
    }
  }, [sessionId]);
  ...
  const flatTasks = useMemo(() => {
    ...
    return PHASES.filter(
      (p) => !collapsedPhases.has(p) && graph.tasks.some((t) => t.phaseType === p),
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "Escape": {
          ...
          if (promptTaskId) {
            setPromptTaskId(null);
          } else if (promptTaskId === null && selectedTask) {
            setSelectedTask(null);
            setPromptTaskId(null);
          }
          break;
  ...
  const phaseL = (c: string, _n: string) =>
    c === "complete"
      ? "var(--color-accent-green)"
      : c === "in_progress"
        ? "var(--color-accent-purple)"
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="bg-accent-purple text-white border border-accent-purple font-medium hover:opacity-88 active:scale-[0.98]"
        >
          Retry
        </button>
      </div>
    ...
    <div style={{ fontFamily: "'JetBrains Mono', monospace", minWidth: 0 }}>
      ...
      <div style={{ borderBottom: "1px solid var(--color-border-subtle)", padding: "20px 0 16px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button
              onClick={() => router.push(`/projects/${sessionId}/summary`)}
            ...
            <TerminalTitle style={{ fontSize: 18, fontWeight: 500, color: "var(--color-text-primary)" }}>
              Task Graph
            </TerminalTitle>
          </div>
      ...
      <div ref={listRef} style={{ marginTop: 8, position: "relative" }}>
        {activePhases.map((phase, phaseIdx) => {
          ...
          const lineColor = nextPhase
            ? phaseL(status, phaseStatus.get(nextPhase) ?? "pending")
            : "transparent";
            ...
            <div key={phase} style={{ display: "flex", gap: 0, position: "relative" }}>
              ...
              <div style={{ flex: 1, minWidth: 0, paddingLeft: 8 }}>
                ...
                >
                  ...
                  <span style={{ color: "var(--color-text-muted)", flexShrink: 0, fontWeight: 400 }}>
                    ({phaseTasks.length})
                  </span>
                  <span
                    style={{
